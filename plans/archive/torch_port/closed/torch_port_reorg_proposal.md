...
**Problem.** `..` holds 32 documents in one flat directory and
`../../../experiments/torch_port` holds 95 scripts plus three directories in
...
## 2. Inventory — `../../../experiments/torch_port` (95 files + 3 dirs)
...
| `results/` | 670 MB, 30 files | **Gitignored** — `../../../../.gitignore` line `../../../experiments/torch_port/results`. Every harness defaults `RESULTS_DIR` to `dirname(__file__)/results`. |
...
## 4. Proposed layout — `../../../experiments/torch_port`
...
`device_policy_design.md`, etc., with no path. Inside `..` there
...
| `../port_plan.md` | 1 | phase0_findings |
...
| `../../../README.md` | 1 | + a paragraph naming 5 files by bare name, **manual rewrite** |
...
`current_plans.md:120` ("reviews archived in `..`" →
`…/reviews/`); `.claude/initial_prompt.md:90` ("findings to
`..`" → `…/active/`); `.claude/initial_prompt.md:89` and
`port_plan.md:11`/`:377` (`../../../experiments/torch_port` — still correct for
...
> the exact path `../../../experiments/torch_port/results`. Leave both alone.
...
4. `plans/current_plans.md:120` — "reviews archived in `..`" →
   `../reviews`.
5. `.claude/initial_prompt.md:90` — "findings to `..`" →
   `../active`.

### 6.5 New files

- `../README.md` — the four-bucket rule, the filename→folder index
  for all 32 documents, and the move-on-close ritual.
- `../../../experiments/torch_port/README.md` — the series map (`mg` live; the six
...
**Is `..` a surface whose paths have been handed to anyone
