...
- `retire_jax.md`: retiring mbirjax as the reference (APPROVED direction 2026-09-13).  Baseline
  archives generated by mbirtorch replace the jax goldens, tolerances are measured
  across platforms, correctness tests without a reference are added, and every
  mbirjax dependency leaves the mbirtorch repository.
