  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.54\textwidth}
      ...
      \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.20\linewidth}@{\hspace{5pt}}>{\raggedright\arraybackslash}p{0.37\linewidth}@{\hspace{5pt}}>{\raggedright\arraybackslash}p{0.36\linewidth}@{}}
        ...
        bandwidth & 546 GB/s, shared (432 GB/s measured on the GPU) & 3.35 TB/s on each GPU \rowrule
...
\begin{frame}{The links differ in speed by a factor of 50}
  ...
  \begin{tikzpicture}[>=Stealth, font=\scriptsize,
    ...
    \foreach \i in {0,1,2,3} {
      \node[gpu] (g\i) at (0.8+1.7*\i, 1.2) {H100\\80 GB};
    ...
    \node[planAccent, anchor=north] at (3.3,0.12) {each GPU reads its own memory at 3.35 TB/s};
    % ---- the M4 Max
  ...
  \noteline{An H100 reads its own memory at 3.35 TB/s.  Data in host memory
  reaches it at 64 GB/s at most, about 50 times slower.  On the Mac a copy from
  host to MPS measured 89 GB/s, about 5 times slower than the GPU's own reads.
  Code developed on a Mac can therefore hide a copy cost that dominates on
  gautschi.}
...
\begin{frame}{Copies: move data once, in large pieces, from pinned memory}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      {\small
      PCIe carries 64 GB/s at most, about 50 times less than the GPU's own
