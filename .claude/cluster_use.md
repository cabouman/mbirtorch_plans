...
| a job dies in seconds with `pip` `OSError: No such file or directory: ...__editable__*.pth` | two jobs ran `pip install -e` into the SAME env at once and one read the torn `.pth` (the torch_p3 sbatch files all install on startup).  Don't launch them simultaneously — chain with `sbatch --dependency=afterany:<jobid>`, which also protects a RUNNING job whose subprocesses re-import the package. |
