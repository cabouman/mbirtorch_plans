...
Record: `kernel_batching_findings.md` (Fable-reviewed).
...
Records: `device_policy_design.md` (rulings) and
...
`../active/multigpu_findings.md` is the results record, and
...
(verdicts LAND NOW; reviews archived in `../reviews`), and all
...
`../active/multigpu_findings.md` carries the read, and the
...
its revisit triggers, is in `kernel_batching_findings.md`.
...
Record: `kernel_sharding_findings.md`.
...
(→ `../phases/phase5_findings.md`); the **mbirtorch slice viewer port**
...
→ `../../projector_kernels/gpu_headroom_findings.md`); the **default partition-sequence
change to `[2, 4, 6, 7]`** (→ `../../partition_sequence`); the **flash-remediation
padding remedies** (→ `../../flash_remediation`); the earlier projector-kernel /
profiling campaign (→ `../../projector_kernels/fwd_back_findings.md`); the multiaxis
vertical-fan path-length factor (shipped); the sparse-projector batching investigation
(closed with code unchanged → `../../projector_batching/batching_refactor_design.md`).
