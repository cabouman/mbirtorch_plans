...
| `back_projection_overview.md` | projector internals (read before touching cone kernels) |
| `sinogram_sharding.md` | parked row-sharding-the-sinogram exploration |
...
- **B4.5 — band-kernel GPU cost (deferred).**  The band (reduce-scatter) kernel is **~2.25×
  ...
  *Alternative axis to consider:* sharding the sinogram by **detector row** instead of by view,
  ...
  analysis in `sinogram_sharding.md`.
...
- **Cone-beam 8-GPU 2048³ recon hang — DIAGNOSED 2026-06-24 as OUT OF MEMORY** (the NCCL "Acquire
  ...
  config worked.  Repro tooling (REMOVED 2026-06-28 once the cone OOM was resolved): `archive/sharding/experiments/cone_deadlock_repro/` (cone-vs-parallel ×
